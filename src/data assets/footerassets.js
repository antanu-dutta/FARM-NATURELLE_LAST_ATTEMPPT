export const footerCategory = [
  { name: "Honey", path: "/collections/honey" },
  { name: "Cooking Oil", path: "/collections/cooking-oil" },
  { name: "A2 Ghee", path: "/collections/a2-ghee" },
  { name: "Apple Cider Vinegar", path: "/collections/apple-cider-vinegar" },
  { name: "Herbal Juices", path: "/collections/herbal-juices" },
  { name: "Hill Turmeric", path: "/collections/hill-turmeric" },
  { name: "Amla Candy", path: "/collections/amla-candy" },
  { name: "Wellness Oils", path: "/collections/wellness-oils" },
  { name: "Stevia", path: "/collections/stevia" }
];

export const footerLinks = [
  { name: "About Us", path: "/about-us" },
  { name: "Our Farms", path: "/our-farms" },
  { name: "Contact Us", path: "/contact-us" },
  { name: "Bulk Purchase", path: "/bulk-purchase" },
  { name: "Blogs", path: "/blogs" },
  { name: "Privacy Policy", path: "/privacy-policy" },
  { name: "Terms and Condition", path: "/terms-and-condition" },
  { name: "Shipping Policy", path: "/shipping-policy" },
  { name: "Refund Policy", path: "/refund-policy" }
]

export const footerCompany = [
  "https://farmnaturelle.com/cdn/shop/files/Link_Amazon_png_480x.png?v=1696490249",
  "https://farmnaturelle.com/cdn/shop/files/FLIPKART_1_480x.png?v=1704798125",
  "https://farmnaturelle.com/cdn/shop/files/image_12383_480x.png?v=1696490466",
  "https://farmnaturelle.com/cdn/shop/files/Link_meesho_png_480x.png?v=1696490466",
  "https://farmnaturelle.com/cdn/shop/files/NicePng_alibaba-logo-png_2803824_1_480x.png?v=1696490466"
]

export const paymentCardIcon = [
  "https://farmnaturelle.com/cdn/shop/t/64/assets/ff-checkout-visa.png?v=77682190091476822571713351401",
  "https://farmnaturelle.com/cdn/shop/t/64/assets/ff-checkout-mastercard.png?v=25567033732622262981713351401",
  "https://farmnaturelle.com/cdn/shop/t/64/assets/ff-checkout-americanexp.png?v=137006734920877356331713351401",
  "https://farmnaturelle.com/cdn/shop/t/64/assets/ff-checkout-paypal.png?v=48995582256179621891713351401",
  "https://farmnaturelle.com/cdn/shop/t/64/assets/ff-checkout-discover.png?v=11980595818515657511713351401",
  "https://farmnaturelle.com/cdn/shop/t/64/assets/ff-checkout-shopify.png?v=48862370562683140221713351401",
  "https://farmnaturelle.com/cdn/shop/t/64/assets/ff-checkout-7.png?v=15029955320502882341713351401"
]